...
for indexGroup = 1 : nOfGroups
    ...
    numberOftypes = length(allLabels)-1;
    overall_matrix = zeros(numberOftypes);
    % read channels
    for indexFile = 1  : nOfFiles
        ...
        perm_matrix = importdata(fullfile(output_folder,strcat(finalScanName,'_perm.mat')));
        orig_matrix = importdata(fullfile(output_folder,strcat(finalScanName,'.mat')));
        h = orig_matrix./ (perm_matrix+eps);
        overall_matrix = overall_matrix +h;
    end
    overall_matrix_normalized = overall_matrix;
    
    for i = 1: size(overall_matrix,1)
        overall_matrix_normalized(i,:) = zscore(overall_matrix(i,:));
    end
    for i = 1: size(overall_matrix,2)
        overall_matrix_normalized(:,i) = zscore(overall_matrix(:,i));
    end
    allLabels_sub = allLabels;
    allLabels_sub(length(allLabels)) = [];
    figure('units','normalized','outerposition',[0 0 1 1])
    heatmap(allLabels_sub,allLabels_sub,overall_matrix_normalized)
    colormap redblue
    %caxis([-5,5])
