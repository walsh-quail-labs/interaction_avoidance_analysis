function [Matrix_Delta,Matrix_low,Unique_all,Unique_low_all,Matrix_high,Unique_high_all,pheno_name,Matrix_Delta_cut_noNaN,Unique_all_string_names_cut]...
    = Heatmap_individual_images_ours(parfor_gates_high,parfor_gates_low,permutations,custom_gatesfolder,Special_clusters_name,Extra_information,pVal_sig)
...
if size(Matrix_Delta_cut_noNaN,1) == 1
    ...
    Delta = clustergram(Matrix_Delta_cut_noNaN,'ColumnLabels',Unique_all_string_names_cut,...
        'Cluster','all','linkage','ward','Colormap',redbluecmap); addTitle...
        (Delta,['Deltaclustergram_Perm_',num2str(permutations),'_',pheno_name,'_',Extra_information]);
...
%         num2str(pixelexpansion),'_Perm_',num2str(permutations),'_',pheno_name,'_',Extra_information]);
    ...
    saveas(plot_delta,fullfile(custom_gatesfolder,['Clustergram_all_PatchDetection_Perm_',num2str(permutations),'_',pheno_name,'_',Extra_information,'significance_cutoff',num2str(pVal_sig),'.fig']));
...
%         'Cluster','all','linkage','ward','Colormap',redbluecmap); addTitle(higher_special,['Delta_Special_Pixel',num2str(pixelexpansion),'_Perm_',permutations,'_',pheno_name,'_',Special_clusters_name,'_',Extra_information]);
    higher_special = clustergram(Matrix_cut_special_noNaN,'ColumnLabels',Special_for_columnlabel_cut,...
        'Cluster','all','linkage','ward','Colormap',redbluecmap); addTitle(higher_special,['Delta_Special_Perm_',permutations,'_',pheno_name,'_',Special_clusters_name,'_',Extra_information]);
    
    plot_delta_special =plot(higher_special);
    
    % Save special output clustergram
    saveas(plot_delta_special,fullfile(custom_gatesfolder,['Clustergram_special_PatchDetection_Perm_',num2str(permutations),'_',pheno_name,'_SpecialCluster',Special_clusters_name,'_',Extra_information,'significance_cutoff',num2str(pVal_sig),'.fig']));
...
%     if strcmp(save_data_forInteractive,'Yes')
        save(fullfile(custom_gatesfolder,['Data_for_interactive_clustergram_PatchDetection_Perm',num2str(permutations),'_',pheno_name,'_',Extra_information,'significance_cutoff',num2str(pVal_sig),'.mat']))
%     end
end
end
