...
function heatMapMatrix = generate_heatmap_per_scan_filepath_with_config_outliers(CombinedFlag,panoFilePath,cellTypeFilePath,allRegIndsPath,IndicatorsList,allCellTypes)
...
%                 histogram(V)
                V = sort(V);
                N_V = length(V);
%                 st_ind = max(floor(0.05*N_V),1);
                en_ind = max(floor(0.95*N_V),1);
%                 figure;
%                 histogram(V(1:en_ind));

                cellMeanIntensityMatrix(i,j) = mean(V(1:en_ind));
            end
        end
    end
