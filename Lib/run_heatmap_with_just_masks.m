...
clc;
addpath(genpath('Lib'))
config_file_path = 'config_mask_heatmap.csv';
...
input_pano_folder = extractBetween(CSVData{lineCounter},k+length('input_pano_folder')+1,length(CSVData{lineCounter}));
...
k = strfind(CSVData{lineCounter},'input_cell_type_folder');
input_celltype_folder = extractBetween(CSVData{lineCounter},k+length('input_cell_type_folder')+1,length(CSVData{lineCounter}));
...
k = strfind(CSVData{lineCounter},'allCellTypeLocation');
allCellTypesAddress = extractBetween(CSVData{lineCounter},k+length('allCellTypeLocation')+1,length(CSVData{lineCounter}));
...
%             heatMapMatrix = generate_heatmap_per_scan_filepath_with_config(CombinedFlag,panoFilePath,cellTypeFilePath,allRegIndsPath,IndicatorsList,allLabels);
            heatMapMatrix = generate_cell_intensity_filepath_with_config_mask_celltype(CombinedFlag,panoFilePath,cellTypeFilePath,allRegIndsPath,IndicatorsList,TypeList,JMF_folderpath,input_pano_folder,allLabels);
