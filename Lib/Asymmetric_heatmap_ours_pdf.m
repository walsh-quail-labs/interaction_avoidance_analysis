function [Delta_allvsall] = Asymmetric_heatmap_ours_pdf(~,~,...
    Matrix_high,Matrix_low,Unique_high_all,Unique_low_all,pheno_name,...
    pixelexpansion,permutations,custom_gatesfolder,Extra_information,pVal_sig,cut_off_percent,allLabels,GroupName)
...
% max(max(Delta_allvsall))
% min(min(Delta_allvsall))
size(Delta_allvsall)
imagesc(Delta_allvsall);
% imagesc(real(abs(Delta_allvsall).^.6).*sign(Delta_allvsall));

colormap(b2r(min(min(Delta_allvsall)),max(max(Delta_allvsall))));
title(['Heatmap_Pixel',num2str(pixelexpansion),'_',Extra_information,'_PatchDetection_Perm_',permutations,'_',pheno_name]);
set(gca,'Xtick',1:length(x),'Ytick',1:length(y),'XtickLabel',allLabels,'YtickLabel',allLabels);
...
saveas(gca,[custom_gatesfolder,'/',strcat('Interaction_Avoidance_Heatmap_',GroupName),'.pdf']);
