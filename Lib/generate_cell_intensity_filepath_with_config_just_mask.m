...
% CombinedFlag,panoFilePath,cellTypeFilePath,allRegIndsPath,IndicatorsList,allCellTypes
function heatMapMatrix = generate_cell_intensity_filepath_with_config_just_mask(CombinedFlag,panoFilePath,cellTypeFilePath,allRegIndsPath,IndicatorsList,TypeList,JMF_folderpath,input_pano_folder,allCellTypes)
...
%         allChanelImages{lt} = imreadTextChannelRaw(Pano,headerIndex);  
    ...
    for i = 1 : nOfCells
        for j = 1 : nOfInds
            ...
            Ch = double(imread(allJMFilePaths{lt}));  
...
%             headerIndex = find(contains(Pano.textdata,lookupQuery));
%             allChanelImages{lt} = imreadTextChannelRaw(Pano,headerIndex);  
            JMF_filePath = extractBetween(panoFilePath{fileITR},length(input_pano_folder)+1,length(panoFilePath{fileITR})-4);
            JMF_filePath = JMF_filePath{1};
            JMF_filePath = fullfile(JMF_folderpath,JMF_filePath,strcat(TypeList{lt},'_2.png'));
            allJMFilePaths{lt} = JMF_filePath;
        end
        ...
        for i = 1 : nOfCells
            for j = 1 : nOfInds
                Ch = double(imread(JMF_filePath{fileITR}));  
                if ~isempty(allRegInds{i})
