...
function [heatMapMatrix,numCellTypes] = generate_heatmap_per_scan_filepath_with_config_v2_fix_mean(CombinedFlag,panoFilePath,cellTypeFilePath,allRegIndsPath,IndicatorsList,allCellTypes)
...
if CombinedFlag == 0
    ...
    numCellTypes = zeros(nOfCellTypes,1);
...
%             heatMapMatrix(i,:) = mean(cellMeanIntensityMatrix(headerIndex,:),1);
            heatMapMatrix(i,:) = sum(cellMeanIntensityMatrix(headerIndex,:),1);
            numCellTypes(i) = length(headerIndex);
        end
