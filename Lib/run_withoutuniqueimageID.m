...
%         if csvFilePathSourceFound==0        
            ...
            if CombinedFlag == 0
                heatMapMatrix = Count_pairwise_rel_perImage(nuc_path,celltype_path,allLabels,dist);
                save(matFilePath,'heatMapMatrix');
                
            else
                heatMapMatrix = 0;
                for fileITR = 1 : length(nuc_path)
                    HM = Count_pairwise_rel_perImage(nuc_path{fileITR},celltype_path{fileITR},allLabels,dist);
